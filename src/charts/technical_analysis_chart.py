...
def build_stock_name_map():
    ...
    # 優先嘗試從 CSV 讀取 (Generated by tools/generate_industry_map.py)
    map_file = os.path.join(META_FOLDER, "moneydj_industries_grouped.csv")
    if os.path.exists(map_file):
        try:
            # 改用手動讀取以處理 Jagged Rows (欄位數不固定)
            ...
            print(f"✅ 中文索引建立完成 (來自 CSV, 共 {count} 檔)")
            return
        except Exception as e:
            print(f"⚠️ CSV 讀取失敗: {e}")

    # Fallback to API
    print("⚠️ 找不到 CSV 或讀取失敗，嘗試 API...")
    ...
    try:
        ...
        if "data" in data:
            for item in data["data"]:
                ...
                if name and sid:
                    name = name.replace("股份有限公司", "").replace("公司", "").replace("科技", "科").replace("電子", "電")
                    STOCK_NAME_TO_ID[name] = sid
                    STOCK_ID_TO_NAME[sid] = name
            print(f"✅ 中文索引建立完成 (來自 API, 共 {len(STOCK_NAME_TO_ID)} 檔)")
    except Exception as e:
        print(f"❌ 建立索引失敗: {e}")
...
def initialize_market_matrix():
    ...
    print("⚠️ 無快取或載入失敗，略過全市場排行初始化 (功能將受限)")
...
def calculate_indicators(stock_id, time_frame='D', days=1500):
    ...
    # 擷取最近 N 天/週
    # 注意: 週線 days=1500 會太多，可以除以 5
    limit = days if time_frame == 'D' else int(days/5)
    df_stock = df_stock.iloc[-limit:].copy()

    # 1. Mansfield / IBD (週線時這些指標的意義會變，這邊簡化：如果沒資料就填 NaN)
    # 若是週線，我們暫時不重新計算全市場週線 Rank (太耗時)，直接用日線最新值或填空
    # 這裡選擇：若是週線，這兩個指標暫時顯示 50 (中位數) 或 NaN
...
def draw_chart(stock_id, stock_name, df_full, plot_count, time_frame='D'):
    ...
    # ★ Volume Fix: Convert Shares to Lots (張數)
    ...
    ax_header.text(0.02, 0.36, "開:", fontsize=18, color='gray', transform=ax_header.transAxes)
    ax_header.text(0.06, 0.36, f"{last['Open']}", fontsize=18, weight='bold', color=c_o, transform=ax_header.transAxes)
    ax_header.text(0.20, 0.36, "高:", fontsize=18, color='gray', transform=ax_header.transAxes)
    ax_header.text(0.24, 0.36, f"{last['High']}", fontsize=18, weight='bold', color=c_h, transform=ax_header.transAxes)

    ax_header.text(0.02, 0.12, "低:", fontsize=18, color='gray', transform=ax_header.transAxes)
    ax_header.text(0.06, 0.12, f"{last['Low']}", fontsize=18, weight='bold', color=c_l, transform=ax_header.transAxes)
    ax_header.text(0.20, 0.12, "收:", fontsize=18, color='gray', transform=ax_header.transAxes)
    ...
    # Volume
    ax_vol_info.text(0.01, 0.3, "成交量", fontsize=20, weight='bold', color='black', transform=ax_vol_info.transAxes)
...
async def process_stat_request(session, chat_id, stock_id, stock_name, time_frame='D'):
    """ 
    工作流程：
    1. 在 ThreadPool 計算數據 (不卡 EventLoop)
    2. 在 ThreadPool 畫圖 (不卡 EventLoop)
    3. 透過 aiohttp 回傳
    """
    ...
    try:
        ...
        if df is None:
            await send_message_async(session, chat_id, f"❌ {stock_id} 無資料")
            return
        ...
        if img_path:
            ...
            opp_text = "切換週線" if time_frame == 'D' else "切換日線"
            ...
            my_pid = os.getpid()
            caption = f"📊 {stock_id} {stock_name} ({'日線' if time_frame=='D' else '週線'}) [PID:{my_pid}]"
            
            await send_photo_async(session, chat_id, img_path, caption, keyboard)
            print(f"✅ {stock_id} ({time_frame}) Processed.")
        else:
            await send_message_async(session, chat_id, f"❌ {stock_id} 繪圖失敗")

    except Exception as e:
        traceback.print_exc()
        await send_message_async(session, chat_id, f"❌ Error: {e}")
    ...
    for u in updates:
        ...
        if "message" in u:
            ...
            if text.startswith("K"):
                print(f"📩 CMD: {text}")
                # Parse
                raw = text[1:].strip() # "2330" or "台積電"
                target = raw.split('-')[0] # 忽略 -200 參數 (因為我們現在有按鈕了)
                
                # Name Map
                sid = STOCK_NAME_TO_ID.get(target, target)
                
                if sid == "TAIEX" or (sid.isdigit() and len(sid)==4):
                    sname = get_stock_name_sync(sid) if sid!="TAIEX" else "加權指數"
...
async def main():
    print("🚀 Async Chart Bot Starting...")
    ...
    async with aiohttp.ClientSession() as session:
        print("✅ Polling started. Waiting for commands (K2330)...")
        while True:
            try:
